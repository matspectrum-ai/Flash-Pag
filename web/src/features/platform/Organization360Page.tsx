  ...
  return (
    <div className="page-stack admin-finance-page">
      ...
      <section className="panel">
        ...
        <div className="merchant-360-members">{members.map((member) => <article key={member.user_id}><span className="customer-avatar"><Users size={14} /></span><div><strong>{member.email || 'Usuário sem e-mail disponível'}</strong></div><span>{roleLabel(member.role)}</span></article>)}{!members.length && membersQuery.isError ? <div className="empty-state compact-empty"><CircleAlert size={22} /><strong>Usuários indisponíveis</strong><span>Não foi possível carregar os vínculos desta conta comercial.</span></div> : null}{!members.length && !membersQuery.isLoading && !membersQuery.isError ? <div className="empty-state compact-empty"><Users size={22} /><strong>Sem usuários carregados</strong><span>Nenhum vínculo foi retornado para esta conta comercial.</span></div> : null}</div>
      </section>
