...
func (s *PostgresStore) Close() {
	if s != nil && s.pool != nil { s.pool.Close() }
}
...
func (s *PostgresStore) FindUserByUsername(ctx context.Context, usernameNormalized string) (User, error) {
	...
	if err != nil { return User{}, fmt.Errorf("find auth user: %w", err) }
	return user, nil
}

func (s *PostgresStore) CreateUser(ctx context.Context, user User, credential Credential) error {
	tx, err := s.pool.Begin(ctx)
	if err != nil { return fmt.Errorf("begin auth user transaction: %w", err) }
	...
	if err := tx.Commit(ctx); err != nil { return fmt.Errorf("commit auth user transaction: %w", err) }
	return nil
}
...
func (s *PostgresStore) GetCredential(ctx context.Context, userID string) (Credential, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return Credential{}, ErrInvalidCredentials }
	if err != nil { return Credential{}, fmt.Errorf("find password credential: %w", err) }
...
func (s *PostgresStore) CreateSession(ctx context.Context, userID, tokenHash, ipHash, userAgentHash string, expiresAt time.Time) error {
	...
	if err != nil { return fmt.Errorf("create auth session: %w", err) }
	return nil
}
...
func (s *PostgresStore) FindSession(ctx context.Context, tokenHash string, now time.Time) (Session, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return Session{}, ErrInvalidCredentials }
	if err != nil { return Session{}, fmt.Errorf("find auth session: %w", err) }
	return session, nil
}
...
func (s *PostgresStore) AllowLogin(ctx context.Context, keyHash, usernameHash, ipHash string, now time.Time) (bool, error) {
	...
	if err != nil { return false, fmt.Errorf("check auth rate limit: %w", err) }
	if blockedUntil != nil && now.Before(blockedUntil.UTC()) { return false, nil }
	return true, nil
}
...
func (s *PostgresStore) RecordLoginFailure(ctx context.Context, keyHash, usernameHash, ipHash string, now time.Time) error {
	...
	if err != nil { return fmt.Errorf("record auth failure: %w", err) }
	return nil
}
...
func (s *PostgresStore) ResetLoginFailures(ctx context.Context, keyHash string) error {
	...
	if err != nil { return fmt.Errorf("reset auth rate limit: %w", err) }
	return nil
}
