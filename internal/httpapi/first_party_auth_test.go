...
func (m *firstPartyMemoryStore) FindUserByUsername(_ context.Context, username string) (auth.User, error) {
	...
	if !ok { return auth.User{}, auth.ErrInvalidCredentials }
	return user, nil
}

func (m *firstPartyMemoryStore) CreateUser(_ context.Context, user auth.User, credential auth.Credential) error {
	if _, ok := m.users[user.Username]; ok { return auth.ErrUserExists }
...
func (m *firstPartyMemoryStore) GetCredential(_ context.Context, userID string) (auth.Credential, error) {
	...
	if !ok { return auth.Credential{}, auth.ErrInvalidCredentials }
	return credential, nil
}

func (m *firstPartyMemoryStore) CreateSession(_ context.Context, userID, tokenHash, _, _ string, expiresAt time.Time) error {
	m.sessions[tokenHash] = firstPartySessionRow{userID: userID, expiresAt: expiresAt, aal: "aal1"}
	return nil
}
...
func (m *firstPartyMemoryStore) FindSession(_ context.Context, tokenHash string, now time.Time) (auth.Session, error) {
	row, ok := m.sessions[tokenHash]
	if !ok || row.revokedAt != nil || !now.Before(row.expiresAt) { return auth.Session{}, auth.ErrInvalidCredentials }
	for _, user := range m.users {
		if user.ID == row.userID { return auth.Session{User: user, AAL: row.aal}, nil }
	}
	return auth.Session{}, errors.New("user not found")
}
...
func (m *firstPartyMemoryStore) AllowLogin(_ context.Context, keyHash, _, _ string, now time.Time) (bool, error) {
	...
	if !ok || !now.Before(row.windowStart.Add(15*time.Minute)) { return true, nil }
...
func (m *firstPartyMemoryStore) RecordLoginFailure(_ context.Context, keyHash, _, _ string, now time.Time) error {
	row, ok := m.rates[keyHash]
	if !ok || !now.Before(row.windowStart.Add(15*time.Minute)) { row = firstPartyRateRow{windowStart: now} }
	row.failures++
	if row.failures >= 5 { blocked := now.Add(15 * time.Minute); row.blockedUntil = &blocked }
	m.rates[keyHash] = row
	return nil
}
...
func TestFirstPartyLoginMeLogoutLifecycle(t *testing.T) {
	...
	if len(cookie) != 1 || cookie[0].Name != firstPartySessionCookie || cookie[0].Value == "" || !cookie[0].HttpOnly || cookie[0].SameSite != http.SameSiteLaxMode { t.Fatalf("unexpected session cookie: %#v", cookie) }

	meReq := httptest.NewRequest(http.MethodGet, "/auth/me", nil)
	meReq.AddCookie(cookie[0])
	meResp := httptest.NewRecorder()
	s.Handler().ServeHTTP(meResp, meReq)
	if meResp.Code != http.StatusOK || !strings.Contains(meResp.Body.String(), `"username":"mateus"`) { t.Fatalf("me status/body = %d/%s", meResp.Code, meResp.Body.String()) }

	logoutReq := httptest.NewRequest(http.MethodPost, "/auth/logout", nil)
	logoutReq.AddCookie(cookie[0])
	logoutResp := httptest.NewRecorder()
	s.Handler().ServeHTTP(logoutReq, logoutResp)
	if logoutResp.Code != http.StatusNoContent { t.Fatalf("logout status = %d, want %d", logoutResp.Code, http.StatusNoContent) }
	...
	if meAfterLogoutResp.Code != http.StatusUnauthorized { t.Fatalf("me after logout status = %d, want %d", meAfterLogoutResp.Code, http.StatusUnauthorized) }
}
...
func TestFirstPartyLoginFailureDoesNotSetCookie(t *testing.T) {
	...
	if resp.Code != http.StatusUnauthorized { t.Fatalf("status = %d, want %d", resp.Code, http.StatusUnauthorized) }
	if got := resp.Result().Cookies(); len(got) != 0 { t.Fatalf("unexpected cookies on failed login: %#v", got) }
}
